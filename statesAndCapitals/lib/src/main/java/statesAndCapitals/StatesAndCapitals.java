...
public class StatesAndCapitals
{
    ...
    public void testStatesAndCapitals() throws IOException
    {
        ...
        List<StateInfo> firstFiveStates = states.stream()
                        .limit(5)
        ...
        List<StateInfo> lastFiveStates = states.stream()
                        .skip(states.size() - 5)
                                .collect(Collectors.toList());
        ...
        List<Integer> firstFiveNumbers = IntStream.range(1, 20)
                .limit(5)
                .boxed()
                .collect(toList());
        ...
        Long statesNumber = states.stream()
                        .count();
        ...
        Boolean isAnyStateLessThan0Elevation = states.stream()
        ...
        Boolean doNoStatesHaveAOneWordMotto = states.stream()
                        .noneMatch(state -> state.getStateMotto() != null && state.getStateMotto().split("\\s+").length ==1);
        ...
        Double averageYearlyPrecipitationAcrossStateCapitals = states.stream()
                ...
                .mapToDouble(CapitalInfo::getAverageYearlyPrecipitationInInches)
                .average()
                .orElse(0.0);
        ...
        Integer totalYearlyPrecipitationAcrossStateCapitals = states.stream()
                .map(StateInfo::getCapital)
                .mapToInt(CapitalInfo::getAverageYearlyPrecipitationInInches)
                .sum();
        ...
        Map<String, Long> numberOfStatesByTimeZone = states.stream()
                .flatMap(state -> state.getTimeZones().stream())
        ...
        List<String> allDistinctStateBirdsMinusMockingbirds = states.stream()
                .map(StateInfo::getStateBird)
                .filter(stateBird -> !stateBird.toLowerCase().contains("mockingbird"))
                .distinct()
                .collect(Collectors.toList());
